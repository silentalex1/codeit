...
async function loadModel() {
  if (modelLoadAttempted) return { textModel, textTokenizer };
  modelLoadAttempted = true;
  modelLoading = true;

  setImmediate(async () => {
    try {
      textModel = await AutoModelForCausalLM.from_pretrained('Xenova/Phi-3-mini-4k-instruct', { quantized: true });
      textTokenizer = await AutoTokenizer.from_pretrained('Xenova/Phi-3-mini-4k-instruct');
      modelLoading = false;
      while (pendingChatRequests.length > 0) {
        const { resolve } = pendingChatRequests.shift();
        resolve({ textModel, textTokenizer });
      }
    } catch (e) {
      textModel = null;
      textTokenizer = null;
  ...
  return { textModel, textTokenizer };
}

function waitForModel() {
  return new Promise((resolve) => {
    if (!modelLoading && textModel && textTokenizer) {
      resolve({ textModel, textTokenizer });
...
async function generateWithText(prompt) {
  if (!textModel || !textTokenizer) return null;
  try {
    const inputs = textTokenizer(prompt, { return_tensors: 'np' });
    const output = await textModel.generate(inputs, {
      max_new_tokens: 256,
      temperature: 0.7,
      do_sample: true,
      top_p: 0.9,
    ...
    const decoded = textTokenizer.batch_decode(output, { skip_special_tokens: true });
...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    const activeSystem = GEMINI_SIM_SYSTEM;
    let llavaPrompt = `USER: <image>\n${activeSystem}\n\n`;
    messages.forEach(m => {
    ...
    if (loaded && loaded.processor && loaded.vlm) {
      try {
        const timeoutPromise = new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), 28000));
        const raw = await Promise.race([generateWithLLaVA(TRANSPARENT_IMAGE, llavaPrompt), timeoutPromise]);
        if (raw && typeof raw === 'string' && raw.length > 1) {
          reply = raw;
        }
      } catch (e) {
  ...
  try {
    ...
    if (loaded && loaded.processor && loaded.vlm) {
      ...
      messages.forEach(m => {
        ...
        llavaPrompt += `${m.role}: ${text}\n`;
      });
      const userText = typeof lastUserMsg.content === 'string' ? lastUserMsg.content : (lastUserMsg.content?.[0]?.text || '');
      llavaPrompt += `User: ${userText}\nASSISTANT:`;

      try {
        const raw = await generateWithLLaVA(TRANSPARENT_IMAGE, llavaPrompt);
        if (raw && raw.length > 1) reply = raw;
      } catch (e) {}
    }
