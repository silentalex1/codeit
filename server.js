...
async function loadModel() {
  if (!generator) {
    try {
      ...
      console.error('Failed to load embedded model (using fallback responses):', e.message);
      generator = null;
    }
  }
  return generator;
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (fastUrl) {
      // Use fast external inference for better speed
      try {
        ...
        console.error('Fast inference failed, using fallback:', e.message);
      }
    } else {
      // Use embedded model
