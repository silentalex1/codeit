...
app.post('/v1/chat/completions', async (req, res) => {
  try {
    ...
    if (!apiKey) return res.status(400).json({ error: 'No Gemini API key associated with this PrysmisAI account. Please add your Gemini key on the /chat page first.' });
...
app.post('/v1/messages', async (req, res) => {
  try {
    ...
    if (!apiKey) return res.status(400).json({ error: 'No Gemini API key found for this PrysmisAI account. Please add your Gemini API key on the /chat page.' });
