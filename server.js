  ...
  try {
    ...
    if (gen) {
      ...
      try {
        ...
        const genPromise = gen(imageToUse, {
          prompt: llavaPrompt,
          max_new_tokens: 180,
          temperature: 0.65,
          do_sample: true,
        ...
        let raw = '';
        if (result && result[0]) {
          raw = (result[0].generated_text || result[0].text || '').trim();
        }
        reply = raw || reply;
        ...
        if (e.message === 'timeout') {
          reply = "PrysmisAI is still loading. Please try your message again in a few seconds.";
        }
      }
    }
  ...
  try {
    ...
    if (gen) {
      ...
      try {
        ...
        const genPromise = gen(TRANSPARENT_IMAGE, {
          prompt: prompt,
          max_new_tokens: 160,
          temperature: 0.65,
          do_sample: true,
